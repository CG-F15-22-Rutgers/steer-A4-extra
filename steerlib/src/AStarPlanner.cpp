...
namespace SteerLib
{
	...
	bool AStarPlanner::computePath(std::vector<Util::Point>& agent_path,  Util::Point start, Util::Point goal, SteerLib::GridDatabase2D * _gSpatialDatabase, float dt)
	{
		...
		timer_end = clock();
		timer = (float)(timer_end - timer_start)/CLOCKS_PER_SEC;

		//std::cout << " 1st timer" << std::endl;
		//std::cout << timer << std::endl;
		//std::cout << dt << std::endl;
		//std::cout << "weight is   " << weight << std::endl;
		//std::cout << "size of temp is   " << temp.size() << std::endl;
		//std::cout << "size of path is   " << agent_path.size() << std::endl;
		...
		while (weight > 1.0f)
		{
			...
			weight = weight - 1.0f;
			//merge incon and open
			open.insert(open.begin(), incon.begin(), incon.end());
			incon.clear();
			//update f value 
